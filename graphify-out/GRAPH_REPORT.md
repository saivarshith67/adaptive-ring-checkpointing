...
## Corpus Check
- 28 files · ~36,193 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 402 nodes · 849 edges · 23 communities detected
- Extraction: 54% EXTRACTED · 46% INFERRED · 0% AMBIGUOUS · INFERRED: 387 edges (avg confidence: 0.62)
...
1. `CheckpointBitFlipInjector` - 43 edges
2. `MetricsCollector` - 43 edges
3. `main()` - 41 edges
4. `CheckpointManager` - 39 edges
5. `FaultType` - 34 edges
6. `main()` - 32 edges
7. `CheckpointFaultConfig` - 32 edges
8. `FaceForensicsDataset` - 30 edges
9. `HashRingCheckpointManager` - 28 edges
10. `VideoFaceForensicsDataset` - 27 edges
...
Cohesion: 0.04
Nodes (60): get_cifar100_dataset(), ConvergenceMetrics, DistributedMetrics, EpochMetrics, ExperimentSummary, FaultMetrics, HashRingMetrics, MetricsCollector (+52 more)

### Community 1 - "Community 1"
Cohesion: 0.11
Nodes (48): CheckpointBitFlipInjector, CheckpointFaultConfig, FaultType, Inject bit-flips into checkpoint tensors before model/optimizer restore., CheckpointManager, ConvergenceAwareScheduler, ConvergenceFit, COCI-inspired online scheduler for convergence-aware checkpoint timing.      T (+40 more)

### Community 2 - "Community 2"
Cohesion: 0.06
Nodes (47): enforce_determinism(), Force deterministic behavior to isolate bit-flip impact from training noise., Save a checkpoint.          Args:             model: The model to save (or DD, barrier(), cleanup_distributed(), get_local_rank(), get_rank(), get_world_size() (+39 more)
...
Cohesion: 0.11
Nodes (6): ElasticRecaching, Write shard to local NVMe cache with atomic rename pattern., Load shard from local cache with verification., Verify shard integrity by loading and checking., Remove cached files for shards owned by a dead node., ShardManager
...
- **Thin community `Community 18`** (1 nodes): `train_convergence_hashring.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 19`** (1 nodes): `train_convergence_normal.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 20`** (1 nodes): `train_epoch_based.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 21`** (1 nodes): `train_hashring_epoch.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 22`** (1 nodes): `Check if running in distributed mode.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `main()` connect `Community 2` to `Community 0`, `Community 1`, `Community 4`, `Community 6`, `Community 7`?**
...
- **Why does `FaultType` connect `Community 1` to `Community 0`, `Community 2`, `Community 3`, `Community 6`?**
  _High betweenness centrality (0.158) - this node is a cross-community bridge._
- **Are the 31 inferred relationships involving `CheckpointBitFlipInjector` (e.g. with `Distributed training entry point using torchrun.  This script is designed to b` and `Seed worker for reproducible data loading across epochs.     Ensures each worke`) actually correct?**
  _`CheckpointBitFlipInjector` has 31 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 36 inferred relationships involving `main()` (e.g. with `log_on_main()` and `preextract_frames()`) actually correct?**
  _`main()` has 36 INFERRED edges - model-reasoned connections that need verification._
- **Are the 34 inferred relationships involving `CheckpointManager` (e.g. with `Distributed training entry point using torchrun.  This script is designed to b` and `Seed worker for reproducible data loading across epochs.     Ensures each worke`) actually correct?**
  _`CheckpointManager` has 34 INFERRED edges - model-reasoned connections that need verification._