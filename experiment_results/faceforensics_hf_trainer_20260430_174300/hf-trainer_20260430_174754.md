# Experiment Report: faceforensics_hf_trainer_20260430_174300

**Training Mode:** hf-trainer  
**Dataset:** faceforensics  
**Model:** efficientnet_b0  
**Configuration:** 20 epochs, batch size 2  
**Generated:** 2026-04-30 17:47:54

## Training Performance

| Metric | Value |
|--------|-------|
| Best Validation Accuracy | 75.00% |
| Final Validation Accuracy | 70.00% |
| Best Validation Loss | 0.7206 |
| Final Validation Loss | 1.0507 |
| Best Training Loss | 0.2296 |
| Final Training Loss | 0.2296 |
| Total Training Time | 294.2s |

## Checkpointing Overhead

| Metric | Value |
|--------|-------|
| Total Checkpoint Count | 3 |
| Total Checkpoint Time | 2.2s |
| Checkpoint Overhead | 0.8% |
| Checkpoint Backend | hf-trainer |
| Framework-Native Saves | 3 |
| Framework-Native Loads | 1 |
| Artifacts Logged | 0 |
| Framework Delegated | False |
| Last Checkpoint Path | ./checkpoints/hf_trainer/checkpoint_epoch_20.pt |

## Convergence Scheduler Metrics

| Metric | Value |
|--------|-------|
| Current Interval (seconds) | 0.00 |
| Total Convergence Checkpoints | 0 |
| Loss Fit Segments Restarted | 0 |
| Theta1 (Exponential Decay) | N/A |
| Theta2 (Exponential Offset) | N/A |

## Hash Ring Metrics

| Metric | Value |
|--------|-------|
| Local Cache Hits | 0 |
| Local Cache Misses | 0 |
| Cache Size (MB) | 0.0 |
| Max Cache Size (MB) | 0.0 |
| Total Shards | 0 |
| Shards Owned by This Node | 0 |
| Central Storage Loads | 1 |
| Local Cache Loads | 0 |
| Cache Writes | 0 |
| Orphaned Shards | 0 |
| Reassigned Shards | 0 |
| Re-cached Shards | 0 |
| Shard Recovery Count | 0 |
| Total Recovery Time | 0.0s |
| Last Load Source | hf-trainer |

## Fault Tolerance Metrics

| Metric | Value |
|--------|-------|
| Injected Faults | 0 |
| Detected Faults | 1 |
| Recovery Attempts | 1 |
| Successfully Recovered | 1 |
| Recovery Failures | 0 |
| Checkpoint Integrity Failures | 0 |
| Total Recovery Time | 75.3s |
| Runtime Fault Triggered | False |
| Runtime Fault Checkpoint | N/A |
| Runtime Fault Epoch | N/A |
| Runtime Fault Global Step | N/A |
| Resume Attempted | True |
| Resume Succeeded | True |
| Resumed From Epoch | 7 |
| Resumed Checkpoint | N/A |
| Resume Load Source | hf-trainer |
| Time to Resume | 75.3s |
| Rollback Epochs | 0.00 |
| Rollback Steps | 240 |
| Lost Work Time | 0.0s |

## Distributed Training Metrics

| Metric | Value |
|--------|-------|
| World Size | 4 |
| Total Synchronization Operations | 0 |
| Total Synchronization Time | 0.0s |
| Communication Overhead | 0.0% |
