...
# -----------------------------------------------
# FaceForensics++ Training - Framework checkpoint backend
# Shared runner used by the framework-specific scripts.
# -----------------------------------------------

if [ -z "$TRAINING_MODE" ] || [ -z "$TRAIN_SCRIPT" ]; then
    echo "TRAINING_MODE and TRAIN_SCRIPT must be set by the wrapper script."
    exit 1
fi

MODE_LABEL=${MODE_LABEL:-$TRAINING_MODE}
CHECKPOINT_SUFFIX=${CHECKPOINT_SUFFIX:-$TRAINING_MODE}
...
DATASET_PATH=${DATASET_PATH:-"./data/faceforensics/FF++"}
...
CHECKPOINT_DIR=${CHECKPOINT_DIR:-"./checkpoints/$CHECKPOINT_SUFFIX"}
CHECKPOINT_INTERVAL=${CHECKPOINT_INTERVAL:-5}
SPLIT=${SPLIT:-0.8}

USE_VIDEO_MODE=${USE_VIDEO_MODE:-false}
USE_FAST_VIDEO_MODE=${USE_FAST_VIDEO_MODE:-true}
NUM_FRAMES=${NUM_FRAMES:-16}
TEMPORAL_MODEL=${TEMPORAL_MODEL:-"mean"}

ENABLE_FAULT_INJECTION=${ENABLE_FAULT_INJECTION:-false}
AUTO_RESUME=${AUTO_RESUME:-false}
ENABLE_RUNTIME_FAULT_INJECTION=${ENABLE_RUNTIME_FAULT_INJECTION:-false}
RUNTIME_FAULT_AFTER_CHECKPOINTS=${RUNTIME_FAULT_AFTER_CHECKPOINTS:-1}
RUNTIME_FAULT_CRASH_AFTER_INJECTION=${RUNTIME_FAULT_CRASH_AFTER_INJECTION:-true}
FAULT_TYPE=${FAULT_TYPE:-"RANDOM_BIT"}
FAULT_LOCATION=${FAULT_LOCATION:-"model"}
FAULT_PROBABILITY=${FAULT_PROBABILITY:-1.0}
FAULT_BIT_FLIPS=${FAULT_BIT_FLIPS:-1}
FAULT_NUM_PROCESSES=${FAULT_NUM_PROCESSES:-1}
FAULT_LOG_PATH=${FAULT_LOG_PATH:-"$CHECKPOINT_DIR/fault_injection_${CHECKPOINT_SUFFIX}.jsonl"}
FAULT_BIT_RANGE=${FAULT_BIT_RANGE:-""}
FAULT_TARGET_LAYERS=${FAULT_TARGET_LAYERS:-""}
FAULT_SPECIFIC_BIT=${FAULT_SPECIFIC_BIT:-""}
